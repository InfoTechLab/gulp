...
gulp.task('build:html', ['del'], function() {
	return gulp.src('src/html/**/*.html')
		.pipe(replace(/<link rel=['"]stylesheet['"](?: type=['"]text\/css['"])? href=['"]\.\.\/css\/(\w+)\.css['"]\s?\/?>/g, '<link rel="stylesheet" href="../css/$1.min.css">'))
		.pipe(replace(/<script(?: type=['"]text\/javascript['"])? src=['"]\.\.\/js\/(\w+)\.js['"]><\/script>/g, '<script src="../js/$1.min.js"></script>'))
		.pipe(htmlmin({
